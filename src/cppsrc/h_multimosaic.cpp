...
void Mutimosaic(const char *path[], const char *pathout, const char *m_fileop, const char *DEM_PathName, int size)
{
	...
	cols_temp = cols_temp * x_sample * dX;
	rows_temp = rows_temp * y_sample * dY;


	IplImage* temp_Img = new IplImage;
	temp_Img = iu_LoadJpgImage(path[0]);


	IplImage* MosaicImg;
	MosaicImg = iu_CreateImage(cols_temp, rows_temp, temp_Img->nChannels);
	delete[](temp_Img->imageData);
	delete temp_Img;

	IuScalar temp;
	IuScalar s, s1, s2, s3, s4;
	IuScalar result;
	temp.val[0] = 0; temp.val[1] = 0; temp.val[2] = 0;
	for (int i = 0; i < MosaicImg->height; i++)
	{
		for (int j = 0; j < MosaicImg->width; j++)
		{
			iu_Set2D(MosaicImg, i, j, temp);
		}
	...
	for (int t = 0; t < size; t++)
	{
		...
		PixelPoint px;
		for (int i = 0; i < domImg->height - 1; i++)
		{
			for (int j = 0; j < domImg->width - 1; j++)
			{


				s = iu_Get2D(domImg, i, j);
				double l_x = l_X0 + xy[0] * j*cos(kappa) - (-xy[1] * i)*sin(kappa);

				double l_y = l_Y0 + xy[0] * j*sin(kappa) + (-xy[1] * i)*cos(kappa);

				px.x = int(l_x * x_sample) + abs(X0)*x_sample;
				px.y = int(l_y * y_sample) + abs(Y0)*y_sample;

				if (px.x < MosaicImg->width - 1 && px.x >= 0 && px.y < MosaicImg->height - 1 && px.y >= 0 && (s.val[0] != 0 || s.val[1] != 0 || s.val[2] != 0))
				{
					s1 = iu_Get2D(MosaicImg, MosaicImg->height - px.y - 1, px.x);
					if (s1.val[0] == 0 && s1.val[1] == 0 && s1.val[2] == 0)
						iu_Set2D(MosaicImg, MosaicImg->height - px.y - 1, px.x, s);
	...
	IplImage* pRgbaImg = iu_CreateImage(MosaicImg->width, MosaicImg->height, 4);
	for (int i = 0; i < MosaicImg->height; i++)
	{
		for (int j = 0; j < MosaicImg->width; j++)
		{
			s = iu_Get2D(MosaicImg, i, j);
	...
	double tiepoints[6] = { 0.0, 0.0, 0.0, UTM_X0, UTM_Y0, 0.0 };
	double pixscale[3] = { xy[0], xy[1], 0 };

	iu_SaveGTifImage(pathout, pRgbaImg, tiepoints, pixscale, ZONE);


	delete[](MosaicImg->imageData);
	delete MosaicImg;
...
int main(int argc, char* argv[])
{
	if(argc != 6)
	{
		std::cout << "[Usage] DOMmosaic: InputImgList outputImg InputEO InputDem_file InputImgList_size " << std::endl;
		return -1;
	}
	const char* input;
	const char* output;
	...
	Mutimosaic(path_input, output, ao_input, dem_input, _size);
