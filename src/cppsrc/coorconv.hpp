...
class WGSUTMTransform
{
private:
	double m_pi;
	double m_sm_a;
	double m_sm_b;
	double m_sm_EccSquared;
	double m_UTMScaleFactor;
...
public:
	WGSUTMTransform()
	{
		...
		m_sm_a = 6378137.0;
		m_sm_b = 6356752.314;
		m_sm_EccSquared = 6.69437999013e-03;
		m_UTMScaleFactor = 0.9996;
	}
...
*
*/
inline double WGSUTMTransform::DegToRad(double deg)
{
...
*
*/
inline double WGSUTMTransform::RadToDeg(double rad)
{
...
*
* Globals:
*     m_sm_a - Ellipsoid model major axis.
*     m_sm_b- Ellipsoid model minor axis.
*
* Returns:
*     The ellipsoidal distance of the point from the equator, in meters.
*
*/
double WGSUTMTransform::ArcLengthOfMeridian(double phi)
{
	double alpha, beta, gamma, delta, epsilon, n;
	double result;

	/* Precalculate n */
	n = (m_sm_a - m_sm_b) / (m_sm_a + m_sm_b);

	/* Precalculate alpha */
	alpha = ((m_sm_a + m_sm_b) / 2.0) * (1.0 + (pow(n, 2.0) / 4.0) + (pow(n, 4.0) / 64.0));
...
*
*/
inline double WGSUTMTransform::UTMCentralMeridian(int zone)
{
...
*
*/
double WGSUTMTransform::FootpointLatitude(double y)
{
	...
	alpha_ = ((m_sm_a + m_sm_b) / 2.0) * (1 + (pow(n, 2.0) / 4) + (pow(n, 4.0) / 64));
...
*
*/
void WGSUTMTransform::MapLatLonToXY(double phi, double lambda, double lambda0, UTMCoor &xy)
{
	...
	/* Precalculate ep2 */
	ep2 = (pow(m_sm_a, 2.0) - pow(m_sm_b, 2.0)) / pow(m_sm_b, 2.0);

	/* Precalculate nu2 */
	nu2 = ep2 * pow(cos(phi), 2.0);

	/* Precalculate N */
	N = pow(m_sm_a, 2.0) / (m_sm_b* sqrt(1 + nu2));
...
*
*/
void WGSUTMTransform::MapXYToLatLon(double x, double y, double lambda0, WGS84Corr &philambda)
{
	...
	Nf = pow(m_sm_a, 2.0) / (m_sm_b* sqrt(1 + nuf2));
	Nfpow = Nf;
...
*
*/
void WGSUTMTransform::LatLonToUTMXY(double lat, double lon, int zone, UTMCoor &xy)
{
	MapLatLonToXY(lat, lon, UTMCentralMeridian(zone), xy);

	/* Adjust easting and northing for UTM system. */
	xy.x = xy.x * m_UTMScaleFactor + 500000.0;
	xy.y = xy.y * m_UTMScaleFactor;
	if (xy.y < 0.0)
		xy.y += 10000000.0;
}
...
*
*/
void WGSUTMTransform::UTMXYToLatLon(double x, double y, int zone, bool southhemi, WGS84Corr &latlon)
{
	double cmeridian;
